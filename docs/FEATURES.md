...
| ĐX-1 | **Đường "Dịch nhanh"**: một màn — chọn ngôn ngữ + cách dịch + mục đích, thả ảnh **hoặc cả gói**, xong **tự tải về**. Không đặt tên chapter, không tải từng trang, không tự bấm xuất | **LIVE một phần** (22-09). Đã bấm tay trên Chromium thật: đăng nhập → tab mặc định "Dịch nhanh" → chọn gói → bấm Dịch ngay → hiện "Đã xong 0/3 trang" (số 3 tới từ backend thật), 0 lỗi console. ⛔ **Đường tự tải về CHƯA kiểm** — pipeline dừng ở OCR vì venv thiếu `paddleocr`. 20 test frontend xanh. Giữ NGUYÊN hai cổng M10 (khai mục đích, **không chọn sẵn**) và đăng nhập — công cụ đối thủ không có hai cổng này, nhưng bỏ đi để cho giống thì mất đúng phần nhắc trách nhiệm bản quyền. Đường đầy đủ **không bị thay thế**, chỉ đổi tab. Xuất bằng **ZIP** chứ không CBZ (E40: `.cbz` không mở được trên Windows). **KHÔNG có bước tick xác nhận trách nhiệm trước khi xuất** — chủ dự án chốt chấp nhận (22-09); hệ thống không tick hộ, nên không có bước đó nghĩa là KHÔNG CÓ, không phải "máy làm thay". Đường đầy đủ vẫn còn bước này |
| ĐX-2 | **Nhận cả gói `.zip`/`.cbz`** làm đầu vào — một chapter 24 trang từ 24 lượt thao tác xuống còn 1 | **LIVE phần nhận gói** (22-09): gói `.cbz` THẬT (Pepper&Carrot CC BY-SA, tên cố ý lộn xộn `p1/p2/p10` + metadata + rác macOS) → `HTTP 202` trong **0,85 giây**, `so_trang:3 · bo_qua:1`, CSDL ghi `order` 1/2/3 **đúng thứ tự tự nhiên**, 3 file ảnh thật trên đĩa. 23 test thuần + 15 test tích hợp xanh. Chặn: bom giải nén (cộng `file_size` khai trong header **trước khi** bung byte nào), đường dẫn thoát thư mục, quá số trang, gói lồng gói. Thứ tự trang theo **khoá tự nhiên** (`p2` trước `p10`) — sắp sai là hỏng im lặng vì file vẫn xuất ra bình thường. Thất bại thì **lùi sạch**, không để lại chapter dở dang. **PDF CHƯA nhận** — cần thêm phụ thuộc vào ảnh `api`, là quyết định riêng; gửi PDF nhận `422 pdf_chua_ho_tro` nói đúng lý do |
| ĐX-3 | **Chọn engine dịch ngay trên màn upload**, kèm nhãn cái nào tốn tiền — thay vì phải gọi `POST /pages/{id}/retry-translate?engine=…` | **LIVE nửa GHI** (22-09): chạy thật xong, CSDL có `translate_engine_override = google_fast` trên cả 3 trang. ⛔ **Nửa ĐỌC chưa kiểm thật** — job dịch chưa bao giờ được xếp vì pipeline dừng ở OCR; nửa đó mới chỉ có test + đối chứng âm. Sửa một lỗi **có thật, chưa từng lộ**: cột `Page.translate_engine_override` có chỗ GHI từ E19 nhưng pipeline đầy đủ **cố ý không ĐỌC** — mọi lựa chọn engine cho chapter thường rơi vào hư không, không một thông báo lỗi nào. Nay `enqueue_translate_after_inpaint` đọc cột đó. Đã **đối chứng âm**: gỡ bản vá ⇒ 2 test đỏ đúng chữ ký lỗi (`None` thay vì engine đã chọn). Mặc định vẫn là bản **miễn phí** |
